...
window.ShearPhoto.prototype = {
	...
	preview: {
		...
		run: function(arg, thisMain) {
			...
			if (Object.prototype.toString.call(arg.preview) === "[object Array]") {
				...
				if (leng > 0) {
					arg.scope.parentNode.insertAdjacentHTML("afterEnd", '<div id="preview"></div>');
					var HTML = "",
					...
					for (var i = 0; i < leng; i++) {
						...
						HTML += '<p style="line-height: 25px; margin-bottom: 10px;">头像预览:</p><a href="javascript:;" style="width:' + arg.preview[i] + "px;height:" + (p ? Math.round(arg.preview[i] / p) + "px;" : "auto;") + "margin-right:" + margin_right + "px;background-color: #F5F5F5;border: " + borderW + "px solid #F5F5F5;" + '"><img src="' + srcDefault + '"/></a>';
					}
	...
	run: function(ImgUrl, Trues, SetPro) {
		...
		var this_ = this,
			arg = this.arg,
			relatImgUrl = this.HTML5.canvas && Trues ? ImgUrl :  ImgUrl+'?time='+timestamp,
			image = new Image();
		...
		image.onload = function() {
			...
			arg.SelectBox.style.visibility = "visible";
			this_.zoom();
			uploadStatus = true;
			this_.pointhandle(2e3, 10, "可以拖动或拉伸蓝边框进行截图", 1, "#fbeb61", "#3a414c");
			document.getElementById("movebox").style.visibility="visible";
			document.getElementById("change-bgimg").style.background="url('/static/Plugin/shearphoto/shearphoto_common/images/bg_img1.png') no-repeat scroll center center";
			document.getElementById("againIMG").style.background="#10B7F5";
			delete relatImgUrl;
			delete ImgUrl;
		};
	...
	config: function(arg) {
		this.arg = arg;
		arg.Shearbar.style.display = "block";//修改过的。
		...
		this.pointhandle(3e3, 10, "请选择本地照片进行截取头像", 2, "#307ff6", "#fff");
	},
	zoom: function() {
		var this_ = this,
	...
	defaultShear: function() {
		this.arg.Shearbar.style.display = "block";//none
	...
	SendUserMsg: function(msg, n, p, b, c, i, k, f) {
		...
		k ? this.arg.Shearbar.style.display = "block" : this.arg.Shearbar.style.display = "block";//none;
	},
	again: function() {
		...
		this.SendUserMsg("正在为你处理截图，稍等...", 0, 2, "#fbeb61", "#3a414c", true, true, function() {
			...
			this_.MyAjax.carry({
				...
				success: function(serverdata) {
					...
					// window.kfclient_uploadheadsuccess(serverdata[2].ImgUrl);
...
window.ShearPhoto.frameUpImg.prototype = {
	...
	DelCreateUpfile: function() {
		...
		inputfile.setAttribute("id","submit_Btn");
