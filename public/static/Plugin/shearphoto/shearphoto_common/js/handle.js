...
window.ShearPhoto.MINGGE(function() {
	//██████████重要设置████████████████
	var relativeUrl = "/static/Plugin/shearphoto/shearphoto_common"; //你不要在后面加斜杠，系统会自动给你加上斜杠，不信看下面！   index.html的JS引用路径自己改，很简单的说
	/*
...
relativeUrl
	...
	var arg = document.getElementById("shearphoto_main").getAttribute("data-arg"); // arg   
	...
	Shear.config({
		...
		preview: [96], // 开启动态预览图片 (数组成员整数型，禁止含小数点 可选false 和数组)     数组内是宽度设置，没有高度设！因为高度会按比例做事 ，此设置代表预览150 大小的预览图（你可以增加多个预览图,如[100,70,50]），设置越多预览图,shearphoto性能越差！官方不建意你开启这个功能，尽可能请设为preview:false

		/*记住 preview 尽量设false*/

		 //url: relativeUrl + "php/shearphoto.url",
		//php: "/header_upload.php?type=get&arg="+ arg +"&company_id="+ company_id +"&id6d="+id6d,
		url:relativeUrl+"php/shearphoto.php", //后端处理地址，保证正确哦，这是常识，连这个地址都能写错，你就是菜B，已经在本版本中帮你加入相对路径，你基本不用改这里了

		scopeWidth: 242, //可拖动范围宽  也就是"main"对象的初始大小(整数型，禁止含小数点) 宽和高的值最好能一致  

		scopeHeight: 225, //可拖动范围高  也就是"main"对象的初始大小(整数型，禁止含小数点) 宽和高的值最好能一致      
		...
		Min: 60, //截框拉伸或拖拽不能少于多少PX(整数型，禁止含小数点)  

		Max: 500, //一开始启动时，图片的宽和高，有时候图片会很大的，必须要设置一下(整数型，禁止含小数点)，尽可能和scopeWidth值 一致  

		backgroundColor: "url(/static/Plugin/shearphoto/shearphoto/images/right-bg-img-1.png) no-repeat center", //遮层色

		backgroundOpacity: 0.6, //遮层透明度-数字0-1 可选

		Border: 1, //截框的边框大小 0代表动态边框。大于0表示静态边框，大于0时也代表静态边框的粗细值
	...
	Shear.complete = function(serverdata) { //截图成功完成时，由shearphoto.php返回数据过来的成功包
			if(serverdata.status === 'error'){
				Shear.pointhandle(3e3, 10, "请选择正确图片", 0, "#f82373", "#fff");
			}else{
				...
				for (var i = 0; i < length; i++)
				 {
					...
					var ImgUrl = serverdata[i]["ImgUrl"].slice(serverdata[i]['ImgUrl'].indexOf('/Upload'));
					creatImg.src = '/static'+ImgUrl;
				}
				...
				creatImg.innerHTML = '<strong><i></i>恭喜你！截图成功</strong><a href="javascript:;" id="completeA">完成</a>';
				complete.appendChild(creatImg);
				...
				completeA.onclick || (completeA.onclick = function() {
					  ...
					  // window.kfclient_webfaceclose();
					 completeA.onclick = null;
					 this_.arg.scope.removeChild(complete);
					 this_.again();
					 this_.pointhandle(3e3, 10, "截图完成！已返回！", 2, "#fbeb61", "#3a414c");
					 $('#shearphoto_main').toggle();
					 $('.left .worker .avatar img').attr('src','/static/Upload/Avatar/'+serverdata[2].ImgName);
				});
			}
		}
		/*--------------------------------------------------------------截图成功后，返回来的callback-------------------------*/
	...
	var up = new ShearPhoto.frameUpImg({
		...
		HTML5: Shear.HTML5,    //切勿改动这句，不然你他妈又问为什么出错
	...
	Shear.addEvent(document.getElementById("saveShear"), "click", function() { //按下截图事件，提交到后端的shearphoto.php接收
		var w_id = $('.left .worker .name').attr('data-w_id');
		var url = window.location.host;
	Shear.SendPHP({shearphoto:"我要传参数到服端",w_id:w_id,url:url});//我们示例截图并且传参数，后端文件shearphoto.php用 示例：$_POST["shearphoto"] 接收参数，不需要传参数请清空Shear.SendPHP里面的参数示例 Shear.SendPHP();
